...
description: Delegate work to Claude Code subagents that each run in their own background tab in the orchestrator's Herdr workspace, visible and attachable, reporting back as cross-session messages. Use when asked to fan out work across agents, delegate to a general/scout/researcher/worker/reviewer role, spawn a subagent in a Herdr tab, run agents in parallel where the user can watch them, or check on, steer, resume, or stop those children. Requires HERDR_ENV=1.
---
...
| --- | --- | --- | --- |
| `general` | Lightweight general tasks: brainstorm, summarize, or transform provided content | haiku | low |
