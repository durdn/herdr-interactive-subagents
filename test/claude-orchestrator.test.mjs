...
describe("Claude orchestrator no-model CLI", { concurrency: false }, () => {
  ...
  it("reclaims stale registry lock generations and releases each generation for the next writer", () => {
    const fx = fixture();
    const lock = registryPath(fx) + ".lock";
    ...
    ok(run(fx, ["spawn", "--role", "scout", "--name", "after-crash", "--cwd", fx.project]));
    assert.deepEqual(registry(fx).children.map((child) => child.name), ["after-crash"]);

    // A normal release is a token marker, not an unsafe recursive delete. The
    // next write must atomically retire it and preserve the first update.
    ok(run(fx, ["stop", "after-crash"]));
    assert.equal(registry(fx).children[0].name, "after-crash");
    assert.ok(registry(fx).children[0].stoppedAt);

    rmSync(lock, { recursive: true, force: true });
    mkdirSync(lock);
    writeFileSync(join(lock, "owner.json"), JSON.stringify({
      version: 1, token: "old-owner-without-pid", leaseUntil: 0,
    }));
    ok(run(fx, ["forget", "after-crash"]));
    assert.deepEqual(registry(fx).children, []);
  });
  ...
  it("serializes concurrent registry writers without losing either update", async () => {
    ...
    writeFileSync(registryPath(fx), JSON.stringify({ children: [
      { name: "forget-a", sessionId: "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", stoppedAt: "then" },
      { name: "forget-b", sessionId: "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb", stoppedAt: "then" },
    ] }));

    const results = await Promise.all([
      runAsync(fx, ["forget", "forget-a"]),
      runAsync(fx, ["forget", "forget-b"]),
    ]);
  ...
  it("preserves unrelated install surfaces while cleaning only marked commands", () => {
    const fx = fixture();
    const shim = join(fx.home, ".claude", "herdr-subagents", "hs.mjs");
    const skill = join(fx.home, ".claude", "skills", "herdr-subagents");
    const foreignSkill = join(fx.dir, "foreign-skill");
    const commands = join(fx.home, ".claude", "commands");
    mkdirSync(dirname(shim), { recursive: true });
    mkdirSync(dirname(skill), { recursive: true });
    mkdirSync(foreignSkill);
    mkdirSync(commands, { recursive: true });
    const foreignShim = "#!/usr/bin/env node\nconsole.log('mine');\n";
    writeFileSync(shim, foreignShim);

    // Install must preflight the shim and leave the skill surface untouched.
    const install = run(fx, ["install"]);
    assert.equal(install.status, 1);
    assert.match(install.stderr, /not a shim generated by hs\.mjs; refusing to overwrite/);
    assert.equal(readFileSync(shim, "utf8"), foreignShim);
    assert.equal(existsSync(skill), false);

    symlinkSync(foreignSkill, skill, process.platform === "win32" ? "junction" : "dir");
    const marker = "<!-- deployed by hs.mjs; edits belong in the plugin repo -->";
    writeFileSync(join(commands, "subagent.md"), `owned deployment\n${marker}\n`);
    writeFileSync(join(commands, "subagents.md"), "user command\n");

    const uninstall = ok(run(fx, ["uninstall"]));
    assert.match(uninstall.stdout, /skipped .*skills.*not ours/);
    assert.match(uninstall.stdout, /skipped .*hs\.mjs \(not ours\)/);
    assert.equal(readFileSync(shim, "utf8"), foreignShim);
    assert.equal(existsSync(join(skill, "SKILL.md")), false);
    assert.equal(existsSync(skill), true, "foreign skill link must remain");
    assert.equal(existsSync(join(commands, "subagent.md")), false);
    assert.equal(readFileSync(join(commands, "subagents.md"), "utf8"), "user command\n");
  });

  it("uninstalls the current plugin's legacy skill target without touching other files", () => {
    const fx = fixture();
    const skill = join(fx.home, ".claude", "skills", "herdr-subagents");
    mkdirSync(dirname(skill), { recursive: true });
    symlinkSync(
      join(ROOT, "claude-plugin"), skill,
