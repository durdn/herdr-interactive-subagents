...
- a Codex skills-only plugin with a Herdr-backed tab, callback, transcript, and permission launcher; and
...
each child appears as a real tab, a detached watcher sends the leader a static completion event,
and the leader retrieves the child-authored result from its Codex transcript. A small registry in
the system temporary directory preserves parent ownership without dirtying the repository.
